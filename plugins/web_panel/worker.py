...
class ServerWorker(QObject):
    ...
    def run(self):
        try:
            self.log_message.emit('Setting up server...')
            self.server = server_main.setup_server(
                self.host,
                self.port,
                self.password_verifier,
            )
            self.log_message.emit('Server event loop is now running.')
            ...
            self.log_message.emit(f'Server thread exited with an error: {error}')
        finally:
            if self.server:
